...
public class BYBBarGraph {
    ...
    public void setAxisColor(float[] c) {
        if (c.length == 4) axisColor = c;
    }

    public void setVerticalAxis(float min, float max, int numDivs) {
        if (axisMesh == null) axisMesh = new BYBMesh(BYBMesh.LINES);
    ...
    public void makeBox(float[] color) {
        if (boxMesh == null) boxMesh = new BYBMesh(BYBMesh.LINES);
    ...
    public void draw(GL10 gl) {
        mesh.draw(gl);
        if (boxMesh != null) boxMesh.draw(gl);

        if (axisMesh != null) {
            if (axisMesh.getNumVertices() > 0) axisMesh.draw(gl);
        }
    }
}
