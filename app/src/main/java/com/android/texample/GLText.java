...
public class GLText {
    ...
    public boolean load(String file, int size, int padX, int padY) {
        ...
        textureRgn =
            new TextureRegion(textureSize, textureSize, 0, 0, textureSize, textureSize);  // Create Full Texture Region
    ...
    //--Draw Text Rotated --//
    // D: draw text at the specified x,y position rotated by angle degrees
    // A: text - the string to draw
    //    x, y - the x,y position to draw text at (bottom left of text; including descent)
    //    angle - the angle by which the text is drawn
    // R: [none]
    public void draw(String text, float x, float y, float angle) {
        ...
        x += ((chrWidth / 2.0f) - (fontPadX * scaleX)) * cos;  // Adjust Start X
        ...
        for (int i = 0; i < len; i++) {              // FOR Each Character in String
            ...
            batch.drawSprite(x, y, angle, chrWidth, chrHeight, charRgn[c]);  // Draw the Character
            float diff = (charWidths[c] + spaceX) * scaleX;
            x += diff * cos;    // Advance X Position by Scaled Character Width and Angle
            y += diff * sin;    // Advance Y Position by Scaled Character Width and Angle
        }
    }
    ...
    //--Draw Text Centered Rotated--//
    // D: draw text CENTERED at the specified x,y position rotated by angle degrees
    // A: text - the string to draw
    //    x, y - the x,y position to draw text at (bottom left of text)
    //    angle - the angle by which the text is drawn
    // R: the total width of the text that was drawn
    public float drawC(String text, float x, float y, float angle) {
        ...
        float diff = (len / 2.0f);
        draw(text, x - diff * cos, y - ((getCharHeight() / 2.0f) + diff * sin),
            angle); // Draw Text Centered and rotated by angle
        return len;                                                 // Return Length
    }

    public float drawCX(String text, float x, float y, float angle) {
        float rad = angle * ((1.0f / 180.0f) * (float) Math.PI);    // Get Angle in Radians
        float cos = (float) Math.cos(rad);                          // Cosine of Angle
        float sin = (float) Math.sin(rad);                          // Sine of Angle
        float len = getLength(text);                                // Get Text Length
        float diff = (len / 2.0f);
        draw(text, x - diff * cos, y - diff * sin, angle);    // Draw Text Centered (X-Axis Only) rotated by angle
        return len;                                                 // Return Length
    }

    public void drawCY(String text, float x, float y, float angle) {
        float rad = angle * ((1.0f / 180.0f) * (float) Math.PI);    // Get Angle in Radians
        float sin = (float) Math.sin(rad);                          // Sine of Angle
        float len = getLength(text);                                // Get Text Length
        float diff = (len / 2.0f);
        draw(text, x, y - ((getCharHeight() / 2.0f) + diff * sin),
            angle); // Draw Text Centered (Y-Axis Only) and rotated by angle
    }
    ...
    public void drawTexture(int width, int height) {
        drawTexture(width, height, 0);
    }

    //--Draw Font Texture Rotated--//
    // D: draw the entire font texture (NOTE: for testing purposes only)
    // A: width, height - the width and height of the area to draw to. this is used to draw the texture to the top-left corner.
    //    angle - the angle by which the font texture is drawn
    public void drawTexture(int width, int height, float angle) {
        batch.beginBatch(textureId);                  // Begin Batch (Bind Texture)
        batch.drawSprite(textureSize / 2, height - (textureSize / 2), angle, textureSize, textureSize,
            textureRgn);  // Draw
        batch.endBatch();                               // End Batch
    }
}
