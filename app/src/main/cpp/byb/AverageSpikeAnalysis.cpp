...
void AverageSpikeAnalysis::process(const char *filePath, int **inSpikeTrains, const int spikeTrainCount,
    ...
    for (int i = 0; i < spikeTrainCount; i++) {
        ...
        for (int j = 0; j < batchSpikeCount; j++) {
            tmpAvr[i].normAverageSpike[j] = AnalysisUtils::map(tmpAvr[i].averageSpike[j], min, max, 0.0f, 1.0f);
            tmpAvr[i].normTopSTDLine[j] = AnalysisUtils::map(tmpAvr[i].topSTDLine[j], min, max, 0.0f, 1.0f);
            tmpAvr[i].normBottomSTDLine[j] = AnalysisUtils::map(tmpAvr[i].bottomSTDLine[j], min, max, 0.0f, 1.0f);
        }
    }
    delete[] tmp;
    ...
    for (int i = 0; i < spikeTrainCount; i++) {
        ...
        std::copy(tmpAvr[i].normAverageSpike, tmpAvr[i].normAverageSpike + batchSpikeCount, outNormAverageSpike[i]);
        std::copy(tmpAvr[i].normTopSTDLine, tmpAvr[i].normTopSTDLine + batchSpikeCount, outNormTopSTDLine[i]);
        std::copy(tmpAvr[i].normBottomSTDLine, tmpAvr[i].normBottomSTDLine + batchSpikeCount, outNormBottomSTDLine[i]);
    }
